...
async function main() {
  console.log('🔄 Collecting image pool from brain subfolders...');
  const allImages = findImages(BRAIN_DIR);
  
  // Exclude screens, animations, diagrams, test templates
  const excludes = ['screenshot', 'stress', 'chart', 'test', 'builder_in_hardhat', 'cat_', 'hand_drawn', 'feature_'];
  const filtered = allImages.filter(img => {
    const name = img.name.toLowerCase();
    const isTempMedia = img.path.includes('.tempmediaStorage'); // ignore temp UI snaps
    return !excludes.some(ex => name.includes(ex)) && !isTempMedia;
  });

  // Calculate unique MD5 hashes to guarantee distinct image sources
  const uniqueMap = {};
  ...
  console.log(`✅ Collected ${pool.length} unique photos from historical folders.`);

  // Classify all unique images
  const vintageMales = [];
  ...
  for (const img of pool) {
    ...
    // Check directory and name heuristics for gender and vintage styles
    ...
    // Heuristics for vintage style
    const isVintage = pStr.includes('e54c01d6-d0a5-4258-9841-edbe623f2f57') || pStr.includes('8937f345-a84b-497b-a539-095f1ef41fa1') || name.includes('soviet') || name.includes('retro') || name.includes('vintage') || name.includes('old') || name.includes('senior');
  ...
  // To build a 100% unique source list, we will pop elements from pools.
  // This guarantees that no two relative slots ever share the same source file.
  const usedPaths = new Set();
  
  function popUnique(poolList, fallbackList) {
    for (let i = 0; i < poolList.length; i++) {
      const img = poolList[i];
    ...
    for (let i = 0; i < fallbackList.length; i++) {
      const img = fallbackList[i];
      if (!usedPaths.has(img.path)) {
        usedPaths.add(img.path);
        return img;
      }
    }
    throw new Error('Could not find enough unique portrait assets in the pool!');
  }
  ...
  for (const p of PEOPLE) {
    ...
    // 1. Get unique portrait source for avatar
    let src1;
    if (gen <= 1) {
      src1 = isM ? popUnique(vintageMales, pool) : popUnique(vintageFemales, pool);
    } else if (gen === 2) {
      src1 = isM ? popUnique(vintageMales, pool) : popUnique(vintageFemales, pool);
    } else if (gen === 3) {
      src1 = isM ? popUnique(modernMales, pool) : popUnique(modernFemales, pool);
    } else {
      src1 = isM ? popUnique(boys, pool) : popUnique(girls, pool);
    }
    ...
    if (p.deceased) {
      // 2. Get a DIFFERENT unique source for photo_1
      let src2;
      if (gen <= 1) {
        src2 = isM ? popUnique(vintageMales, pool) : popUnique(vintageFemales, pool);
      } else {
        src2 = isM ? popUnique(vintageMales, pool) : popUnique(vintageFemales, pool);
      }
      ...
      // 3. For photo_2, crop-transform src2 to show a close-up face crop of the same person
...
async function processImage(srcPath, destPath, gen, type, key) {
  ...
  if (gen === 0 || gen === 1) {
    ...
    img = img.modulate({ saturation: 0.75, brightness: 1.02 })
             .tint({ r: 125, g: 110, b: 85 }); // 70s fading tint
  } else if (gen === 3) {
    img = img.modulate({ saturation: 1.25, brightness: 1.05 }); // 90s polaroid look
  } else {
    img = img.sharpen();
  }

  // To guarantee unique size and hash for every output, we inject a microscopic transparent watermarking SVB
