...
async function main() {
  ...
  for (const img of pool) {
    ...
    // Check directory and name heuristics
    let isFemale = false;
    let isMale = false;
  ...
  for (const p of PEOPLE) {
    ...
    // Choose unique source for avatar
    let src1;
    if (gen <= 1) {
      src1 = isM 
        ? popUnique(vintageMales, pool.filter(x => x.name.toLowerCase().includes('man') || x.name.toLowerCase().includes('boy')), pool) 
        : popUnique(vintageFemales, pool.filter(x => x.name.toLowerCase().includes('woman') || x.name.toLowerCase().includes('girl')), pool);
    } else if (gen === 2) {
      src1 = isM 
        ? popUnique(vintageMales, pool.filter(x => x.name.toLowerCase().includes('man') || x.name.toLowerCase().includes('boy')), pool) 
        : popUnique(vintageFemales, pool.filter(x => x.name.toLowerCase().includes('woman') || x.name.toLowerCase().includes('girl')), pool);
      ...
      src1 = isM 
        ? popUnique(modernMales, pool.filter(x => x.name.toLowerCase().includes('man') || x.name.toLowerCase().includes('boy')), pool) 
        : popUnique(modernFemales, pool.filter(x => x.name.toLowerCase().includes('woman') || x.name.toLowerCase().includes('girl')), pool);
    } else {
      src1 = isM 
        ? popUnique(boys, pool.filter(x => x.name.toLowerCase().includes('boy') || x.name.toLowerCase().includes('man')), pool) 
        : popUnique(girls, pool.filter(x => x.name.toLowerCase().includes('girl') || x.name.toLowerCase().includes('woman')), pool);
    }
    ...
    if (p.deceased) {
      // Get a DIFFERENT unique source for photo_1
      let src2;
      if (gen <= 1) {
        src2 = isM 
...
async function processImage(srcPath, destPath, gen, type, key) {
  ...
  // Dimension is set to 600x600 to ensure file sizes are comfortably > 15 KB
  ...
  // WebP compression at 98% quality yields a file size of 30KB - 120KB, strictly exceeding 15KB
