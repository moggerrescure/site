...
async function structureFullBiography(rawText, context = {}) {
  ...
  const system = [
    ...
    '8. ОБЯЗАТЕЛЬНО соблюдай хронологический порядок блоков в массиве blocks! Блоки должны идти строго в порядке жизненного пути: childhood (детство) → education (образование) → military (военная служба, если была) → career (карьера/работа) → family (семья) → hobbies (хобби/увлечения) → legacy (наследие/память). Custom-блоки вставляй между ближайшими стандартными разделами по хронологии описываемых в них событий. Например, если custom-блок про переезд был между учёбой и работой — ставь его между education и career.',
    '9. Отвечай строго на русском языке.',
    '',
...
async function reconstructPage(rawText, currentBlocks, currentHeader = {}) {
  ...
  const system = [
    ...
    '- ОБЯЗАТЕЛЬНО соблюдай хронологический порядок команд! Команды ADD и UPDATE должны идти строго в порядке жизненного пути: childhood (детство) → education (образование) → military (военная служба) → career (карьера) → family (семья) → hobbies (хобби) → legacy (наследие). Custom-блоки вставляй между ближайшими стандартными по хронологии событий. Нельзя ставить "работу" перед "образованием" или "детство" после "карьеры".',
    '',
